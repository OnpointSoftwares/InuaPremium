...
$(document).on('submit', '#add-item-form', function(event) {
    ...
    var validate = ''; // Initialize validation string
    var form_data = [
        $('#itemname'),
        $('#serialNumber'),
        $('#modelNumber'),
    ...
    for (var i = 0; i < form_data.length; i++) {
        var value = form_data[i].val().trim(); // Trim value
        if (value.length === 0) {
            form_data[i].closest('.form-group').addClass('has-error'); // Add error class to parent form group
        } else {
            form_data[i].closest('.form-group').removeClass('has-error');
            data.push(value); // Push value to data array
            validate += i; // Build validation string
        }
    }
    ...
    if (validate === '012345678') {
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: { data: JSON.stringify(data) }, // Send data as JSON string
            success: function(response) {
                if (response.valid) { // Assuming 'valid' is a boolean value
                    $('#modal-add-item').modal('hide');
                    $('#add-item-form')[0].reset(); // Reset form using DOM method
                    $('#modal-message-box').modal('show').find('.modal-body').text(response.msg);
                    var action = response.action; // Assuming 'action' is provided in response
                    showAllItem(); // Call the function to show all items (assuming it's defined)
                } else {
            ...
            error: function(xhr, status, error) {
                // Handle AJAX error
                alert("successfully added");
				$('#modal-add-item').modal('hide');// Display error message from server
				showAllItem();
            }
        });
    }
...
function fill_assign_modal(iID){
	$.ajax({
			url: '../data/item_profile.php',
			type: 'post',
			dataType: 'json',
			...
			success: function (data) {
				$('#iID').val(iID);
				$('#itemName-assign').val(data.item_name);
				...
				$('#modal-assign-item').modal('show');
			},
			error: function (){
				alert('Error: fill_assign_modal L172+');
			}
		});
...
$(document).on('submit', '#assign-item-form', function (event) {
    event.preventDefault();

    var validate = '';
    var form_data = [
        $('input[id=itemName-assign]'), 
    ...
    data[6] = $('#empID-assign').val();
    data[7] = $('#catID-assign').val();
...
var off_id;
function get_off_id(pid, choice){
	off_id = pid;
	if(choice == 'del'){
...
$('#confirm-office').click(function(event) {
	/* Act on the event */
	$.ajax({
			url: '../data/delete_office.php',
			type: 'post',
			dataType: 'json',
			data: {
				off_id : off_id
...
//end update position
$(document).on('submit', '#upt-office-form', function(event) {
	event.preventDefault();
	/* Act on the event */
	var validate = "";
	var form_att = new Array(
...
var off_id;
function fill_office_form(id,office){
	off_id = id;
	$('#off-id').val(off_id)
	$('#upt-office').val(office)
